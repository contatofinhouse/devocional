...
Retorne o resultado no formato JSON estrito exatamente com a estrutura abaixo (responda APENAS com o JSON, sem marcações markdown):
{
...
async function runRewrite(isValidation = false) {
  console.log(`🚀 Iniciando reescrita de lições via Claude... (Modo Teste: ${isValidation})`);
  ...
  for (const lesson of targetLessons) {
    ...
    while (!success && retries > 0) {
      try {
        ...
        // Delay to respect API limits (around 1.5 seconds per request)
        await sleep(1500);

      } catch (err) {
        retries--;
        console.warn(`⚠️ Erro ao processar ID: ${id} (${err.message}). Tentando novamente em 5 segundos... (Tentativas restantes: ${retries})`);
        await sleep(5000);
      }
    }
