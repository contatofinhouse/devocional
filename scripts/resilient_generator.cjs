...
function downloadImage(url, destPath) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)' }, timeout: 45000 }, (res) => {
...
async function generateSingleScene(item, index) {
  const pubPath = path.join(outDir, item.file);
  const brainPath = path.join(brainDir, item.file);

  if (fs.existsSync(pubPath)) {
    const stat = fs.statSync(pubPath);
    // If it's already a freshly generated Pixar file (< 100KB), keep it
    if (stat.size < 120000 && stat.size > 20000 && item.file !== 'kids_noe_2_embarque.jpg') {
      console.log(`[SKIP ALREADY GENERATED] ${item.file} (${stat.size} bytes)`);
      if (!fs.existsSync(brainPath)) fs.copyFileSync(pubPath, brainPath);
      return true;
    }
  }
  ...
  for (let attempt = 1; attempt <= 10; attempt++) {
    const url = `https://image.pollinations.ai/prompt/${encoded}?width=1024&height=768&seed=${item.seed}&nologo=true`;
    try {
      ...
      console.log(`  -> [RETRY ${attempt}] ${item.file}: ${err.message} (pausing 10s...)`);
...
async function main() {
  console.log(`Starting resilient Pixar batch generation...`);
  for (let i = 0; i < scenes.length; i++) {
    await generateSingleScene(scenes[i], i);
    await new Promise(r => setTimeout(r, 3000));
  }
  console.log('ALL 25 PIXAR SCENES COMPLETED SUCCESSFULLY!');
}
